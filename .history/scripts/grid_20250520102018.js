        ...
        for(let r = 0; r < this.rows; r++){
            for(let c = 0; c < this.cols; c++){
                ...
                if(this.matriz[r][c] !== 0){
                    const color = this.block.getColorPalette(this.matriz[r][c]);
                    this.drawSquare(position.x, position.y, this.cellSize, color.square, "#000");
                    this.block.drawTriangle(position.x, position.y, position.x + this.cellSize, position.y + this.cellSize, position.x + this.cellSize, position.y, color.rightTriangle);
                    this.block.drawTriangle(position.x, position.y + this.cellSize, position.x + this.cellSize, position.y + this.cellSize, position.x, position.y + this.cellSize, color.leftTriangle);
